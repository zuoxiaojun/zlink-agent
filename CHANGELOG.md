...
- **U9 Cloud（U9C）ERP 支持**：
  ...
  - 12 类预制业务查询（销售订单含行明细、采购订单含行明细、生产订单含产出明细等）
  - 数据字典覆盖 6 张核心业务表
  - 新建会话打招呼消息更新，标注已对接的 U8+ 和 U9 Cloud
- **内置分析技能**：
  - 预置 `iuap-data-reporting` 技能（图文报告生成，支持经营分析/周报/月报/深度洞察）
  - 预置 `iuap-data-visualizing` 技能（纯图表生成，支持 20+ ECharts 图表类型）
- **ERP 门控加固**：
  - `get_instructions_for_query()` 新增 `_erp_enabled` 过滤，禁用的 ERP 技能不会将全量 SKILL.md 注入 system prompt
...
- `web/src/pages/SettingsERPPage.tsx` — toast 位置调整

# Changelog

## v1.11.1 — 2026-08-25 (修复：禁用 ERP 仍出现在 LLM 上下文中)

**范围**: 修复已禁用的 NC ERP 系统仍通过多个来源出现在 LLM 上下文中的问题，以及前端新会话 Token 用量残留。

### 修复

- **ERP 上下文**: 禁用系统不再出现在可用数据源列表（`_build_erp_context`）
- **技能索引**: 禁用的 ERP 对应技能（nc）不再注入 system prompt 技能列表
- **Base prompt**: 移除写死的系统名称，改用通用描述
- **记忆系统**: 清除所有 NC 相关历史摘要和事实记忆
- **前端 Token 残留**: `SET_SESSION` 和 `NEW_SESSION` 均重置 tokenUsage/apiCalls，修复新建对话时显示上一会话用量
...
- **品牌中文化**: loading 页、侧边栏、页面标题、欢迎语全部改为"智链 Agent"
- **NC skill 重写**: 从外部 MCP 子进程改为内置工具（oracledb 直连），增加数据分析流水线章节
- **gitignore 新增**: 忽略 `.pi-glla/` 目录
...
- **总账凭证查询** (`gl_voucher`): GL_VOUCHER + GL_DETAIL 关联，科目经 `BD_ACCASOA → BD_ACCOUNT` 链解析出编码/名称；支持日期段、凭证号过滤；含摘要、借贷金额、作废状态
- **科目余额查询** (`gl_balance`): GL_BALANCE 仅存期间发生额，模板按期初（<起始期间累计净额）/本期借贷/期末自动聚合；参数 year（必填）/period_start/period_end/科目编码模糊
- **字典解析器修正** (`parse_nc_dict_chm.py`): 同名表多页面时从"取第一个"改为"取字段最丰富"——GL_VOUCHER 从 3 字段权限页修正为 68 字段凭证实体，GL_DETAIL 93 字段；新增 `--extra-tables` 跨模块补表（BD_ACCOUNT/BD_ACCASOA 已入库，字典 548 表）
- **nc_query 参数白名单**: 传错参数名不再静默丢弃全部过滤条件退化为全表查询；多余参数忽略、缺必填参数返回明确报错
