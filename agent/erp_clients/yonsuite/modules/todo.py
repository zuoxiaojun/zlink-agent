...
class TodoModule(BaseAPIClient):
    ...
    @retry_on_failure()
    def query_todos(self, access_token: str, page_no: int = 1, page_size: int = 10) -> dict:
        """
        查询用户待办数据列表
    ...
    def query_todos_parsed(self, access_token: str, page_no: int = 1, page_size: int = 10) -> list[TodoItem]:
        """
        查询用户待办数据（解析为模型对象）
    ...
    def query_todo_count(self, access_token: str, status: str = "todo") -> int:
        """
        查询待办/已办数量
