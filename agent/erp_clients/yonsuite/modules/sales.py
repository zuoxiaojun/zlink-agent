...
from ..models import SaleOrder, SaleOrderDetail
...
class SalesModule(BaseAPIClient):
    ...
    def get_order_detail_parsed(self, access_token: str, order_id: str) -> SaleOrderDetail | None:
        """
        查询销售订单详情（解析为模型对象）
    ...
    def format_orders_list(self, orders: list[SaleOrder]) -> str:
        ...
        for order in orders:
            ...
            lines.append(" | ".join(v.ljust(w) for v, w in zip(row, col_widths, strict=False)))
