...
def _build_summary_prompt(
    ...
    return f"""压缩以下对话历史，生成结构化的上下文摘要。使用以下固定格式：
...
## Key Decisions
（做出了哪些关键决策？技术选型、设计方案、约定等）

## Next Steps
（哪些任务待完成？下一步做什么？）

## Critical Context
（继续对话必须知道的重要上下文：当前工作目录、正在修改的文件、注意事项等）
...
_PATH_PATTERNS = [
    ...
    re.compile(r"(?:^|[\s\"'`=,(])(?:~/|/Users/zuoxiaojun/vibecoding/YS-Agent/)([A-Za-z0-9_./-]+\.[A-Za-z0-9]{1,8})"),
