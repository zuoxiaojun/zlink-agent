...
**First time:** `bash agent/skills/minimax-docx/scripts/setup.sh` (or `powershell agent/skills/minimax-docx/scripts/setup.ps1` on Windows, `--minimal` to skip optional deps).

**First operation in session:** `agent/skills/minimax-docx/scripts/env_check.sh` — do not proceed if `NOT READY`. (Skip on subsequent operations within the same session.)
...
```csharp
// File: agent/skills/minimax-docx/scripts/dotnet/task.csx  (or a new .cs in a Console project)
// dotnet run --project agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Cli -- run-script task.csx
...
```bash
dotnet run --project agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Cli --
```
...
│   → Read agent/skills/minimax-docx/references/scenario_a_create.md
│
└─ Has input .docx
    ├─ Replace/fill/modify content → Pipeline B: FILL-EDIT
    │   signals: "fill in", "replace", "update", "change text", "add section", "edit"
    │   → Read agent/skills/minimax-docx/references/scenario_b_edit_content.md
    │
    └─ Reformat/apply style/template → Pipeline C: FORMAT-APPLY
        ...
        → Read agent/skills/minimax-docx/references/scenario_c_apply_template.md
```
...
Convert `.doc` → `.docx` if needed: `agent/skills/minimax-docx/scripts/doc_to_docx.sh input.doc output_dir/`

Preview before editing (avoids reading raw XML): `agent/skills/minimax-docx/scripts/docx_preview.sh document.docx`

Analyze structure for editing scenarios: `$CLI analyze --input document.docx`

## Scenario A: Create

Read `agent/skills/minimax-docx/references/scenario_a_create.md`, `agent/skills/minimax-docx/references/typography_guide.md`, and `agent/skills/minimax-docx/references/design_principles.md` first. Pick an aesthetic recipe from `agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Core/Samples/AestheticRecipeSamples.cs` that matches the document type — do not invent formatting values. For CJK, also read `agent/skills/minimax-docx/references/cjk_typography.md`.

**Choose your path:**
- **Simple** (plain text, minimal formatting): use CLI — `$CLI create --type report --output out.docx --config content.json`
- **Structural** (custom styles, multi-section, TOC, images, complex tables): write C# directly. Read the relevant `agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Core/Samples/*.cs` first.
...
Read `agent/skills/minimax-docx/references/scenario_b_edit_content.md` first. Preview → analyze → edit → validate.

**Choose your path:**
- **Simple** (text replacement, placeholder fill): use CLI subcommands.
- **Structural** (add/reorganize sections, modify styles, manipulate tables, insert images): write C# directly. Read `agent/skills/minimax-docx/references/openxml_element_order.md` and the relevant `Samples/*.cs`.
...
Read `agent/skills/minimax-docx/references/scenario_c_apply_template.md` first. Preview and analyze both source and template.
...
agent/skills/minimax-docx/scripts/docx_preview.sh doc.docx
# Verify: font contamination=0, table count correct, drawing count correct, sectPr count correct
```

Final preview: `agent/skills/minimax-docx/scripts/docx_preview.sh doc.docx`
...
**Multi-template merge:** When given multiple template files (font, heading, breaks), read `agent/skills/minimax-docx/references/scenario_c_apply_template.md` section "Multi-Template Merge" FIRST. Key rules:
...
- See `agent/skills/minimax-docx/references/scenario_c_apply_template.md` section "Multi-Section Header/Footer Transfer".
...
| File | When |
|------|------|
| `agent/skills/minimax-docx/references/scenario_a_create.md` | Pipeline A: creating from scratch |
| `agent/skills/minimax-docx/references/scenario_b_edit_content.md` | Pipeline B: editing existing content |
| `agent/skills/minimax-docx/references/scenario_c_apply_template.md` | Pipeline C: applying template formatting |

### C# code samples (compilable, heavily commented — read when writing code)

| File | Topic |
|------|-------|
| `agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Core/Samples/DocumentCreationSamples.cs` | Document lifecycle: create, open, save, streams, doc defaults, settings, properties, page setup, multi-section |
| `agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Core/Samples/StyleSystemSamples.cs` | Styles: Normal/Heading chain, character/table/list styles, DocDefaults, latentStyles, CJK 公文, APA 7th, import, resolve inheritance |
| `agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Core/Samples/CharacterFormattingSamples.cs` | RunProperties: fonts, size, bold/italic, all underlines, color, highlight, strike, sub/super, caps, spacing, shading, border, emphasis marks |
| `agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Core/Samples/ParagraphFormattingSamples.cs` | ParagraphProperties: justification, indentation, line/paragraph spacing, keep/widow, outline level, borders, tabs, numbering, bidi, frame |
| `agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Core/Samples/TableSamples.cs` | Tables: borders, grid, cell props, margins, row height, header repeat, merge (H+V), nested, floating, three-line 三线表, zebra striping |
| `agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Core/Samples/HeaderFooterSamples.cs` | Headers/footers: page numbers, "Page X of Y", first/even/odd, logo image, table layout, 公文 "-X-", per-section |
| `agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Core/Samples/ImageSamples.cs` | Images: inline, floating, text wrapping, border, alt text, in header/table, replace, SVG fallback, dimension calc |
| `agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Core/Samples/ListAndNumberingSamples.cs` | Numbering: bullets, multi-level decimal, custom symbols, outline→headings, legal, Chinese 一/（一）/1./(1), restart/continue |
| `agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Core/Samples/FieldAndTocSamples.cs` | Fields: TOC, SimpleField vs complex field, DATE/PAGE/REF/SEQ/MERGEFIELD/IF/STYLEREF, TOC styles |
| `agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Core/Samples/FootnoteAndCommentSamples.cs` | Footnotes, endnotes, comments (4-file system), bookmarks, hyperlinks (internal + external) |
| `agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Core/Samples/TrackChangesSamples.cs` | Revisions: insertions (w:t), deletions (w:delText!), formatting changes, accept/reject all, move tracking |
| `agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Core/Samples/AestheticRecipeSamples.cs` | 13 aesthetic recipes from authoritative sources: ModernCorporate, AcademicThesis, ExecutiveBrief, ChineseGovernment (GB/T 9704), MinimalModern, IEEE Conference, ACM sigconf, APA 7th, MLA 9th, Chicago/Turabian, Springer LNCS, Nature, HBR — each with exact values from official style guides |

Note: `Samples/` path is relative to `agent/skills/minimax-docx/scripts/dotnet/MiniMaxAIDocx.Core/`.

### Markdown references (read when you need specifications or design rules)

| File | When |
|------|------|
| `agent/skills/minimax-docx/references/openxml_element_order.md` | XML element ordering rules (prevents corruption) |
| `agent/skills/minimax-docx/references/openxml_units.md` | Unit conversion: DXA, EMU, half-points, eighth-points |
| `agent/skills/minimax-docx/references/openxml_encyclopedia_part1.md` | Detailed C# encyclopedia: document creation, styles, character & paragraph formatting |
| `agent/skills/minimax-docx/references/openxml_encyclopedia_part2.md` | Detailed C# encyclopedia: page setup, tables, headers/footers, sections, doc properties |
| `agent/skills/minimax-docx/references/openxml_encyclopedia_part3.md` | Detailed C# encyclopedia: TOC, footnotes, fields, track changes, comments, images, math, numbering, protection |
| `agent/skills/minimax-docx/references/typography_guide.md` | Font pairing, sizes, spacing, page layout, table design, color schemes |
| `agent/skills/minimax-docx/references/cjk_typography.md` | CJK fonts, 字号 sizes, RunFonts mapping, GB/T 9704 公文 standard |
| `agent/skills/minimax-docx/references/cjk_university_template_guide.md` | Chinese university thesis templates: numeric styleIds (1/2/3 vs Heading1), document zone structure (cover→abstract→TOC→body→references), font expectations, common mistakes |
| `agent/skills/minimax-docx/references/design_principles.md` | **Aesthetic foundations**: 6 design principles (white space, contrast/scale, proximity, alignment, repetition, hierarchy) — teaches WHY, not just WHAT |
| `agent/skills/minimax-docx/references/design_good_bad_examples.md` | **Good vs Bad comparisons**: 10 categories of typography mistakes with OpenXML values, ASCII mockups, and fixes |
| `agent/skills/minimax-docx/references/track_changes_guide.md` | Revision marks deep dive |
| `agent/skills/minimax-docx/references/troubleshooting.md` | **Symptom-driven fixes**: 13 common problems indexed by what you SEE (headings wrong, images missing, TOC broken, etc.) — search by symptom, find the fix |
