...
name: data-analysis
title: data-analysis
description: 数据分析技能，支持 KPI 分析、报表洞察、统计分析和决策简报；用于销售/采购/生产报表的决策简报生成
---
...
本技能专注于数据分析方法，不直接查询数据源。配合 `yonsuite-skill` 使用：
1. 先用 `yonsuite-skill` 查询 YonSuite API 获取业务数据
2. 再用本技能进行统计分析并生成决策简报
