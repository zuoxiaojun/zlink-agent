...
def _lighten(hex_color: str, factor: float = 0.09) -> str:
    ...
    return f"#{round(r * factor + 255 * (1 - factor)):02X}{round(g * factor + 255 * (1 - factor)):02X}{round(b * factor + 255 * (1 - factor)):02X}"
...
def main():
    ...
    parser.add_argument(
        "--type", default="general", choices=list(PALETTES.keys()), help="Document type: " + ", ".join(PALETTES.keys())
    )
    ...
    parser.add_argument(
        "--accent",
        default="",
        help="Override accent colour (hex, e.g. #2D6A8F). accent_lt is auto-derived by lightening toward white.",
    )
    parser.add_argument("--cover-bg", default="", help="Override cover background colour (hex).")
