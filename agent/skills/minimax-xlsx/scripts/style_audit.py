...
def _fmt_is_comma(num_fmt_id: int, num_fmts: dict) -> bool:
    ...
    return (
        "#,##" in fmt_code
        and not fmt_code.endswith(",")
        and not fmt_code.endswith(',"M"')
        and not fmt_code.endswith(',"K"')
    )
                        ...
                        w.append(
                            {
                                ...
                                "If intended rate is ~{:.0f}%, store as {:.4f} instead.".format(pct_val, pct_val / 100),
                            }
                        )
                except (ValueError, TypeError):
                    pass
...
def main() -> None:
    ...
    print(f"Cells   : {s['total_cells_inspected']} inspected  ({s['formula_cells']} formula, {s['input_cells']} input)")
    ...
    if not summary_only:
        if results["violations"]:
            ...
            for item in results["violations"]:
                ...
                if t == "count_mismatch":
                    print(
                        f"  [FAIL] {item['element']} count mismatch: declared={item['declared']}, "
        ...
        if results["warnings"] and not summary_only:
            ...
            for item in results["warnings"]:
                ...
                if t == "numeric_input_may_lack_blue":
                    print(
                        f"  [WARN] [{item['sheet']}!{item['cell']}] numeric value={item['value']} "
                        f"has black font — if user-editable assumption, use blue-font input style"
                    )
                elif t == "percent_value_gt_1":
                    print(
                        f"  [WARN] [{item['sheet']}!{item['cell']}] percent-format cell has "
