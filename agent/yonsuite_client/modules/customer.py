...
class CustomerModule(BaseAPIClient):
    ...
    @retry_on_failure()
    def query_customer_details_batch(self, access_token: str,
    ...
    def query_customer_detail_single(self, access_token: str,
        ...
        result = self.query_customer_details_batch(
            access_token, customer_list, belong_org_id, belong_org_code
        )
    ...
    def format_customer_detail(self, detail: Dict) -> str:
        ...
        if address_infos:
            ...
            for i, addr in enumerate(address_infos[:3], 1):
                lines.append(f"  {i}. {addr.get('address', 'N/A')} - {addr.get('receiver', 'N/A')} {addr.get('mobile', 'N/A')}")
