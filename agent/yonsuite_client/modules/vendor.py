...
class VendorModule(BaseAPIClient):
    ...
    @retry_on_failure()
    def get_vendor_detail(self, access_token: str, 
                          vendor_id: str,
    ...
    def query_vendors_parsed(self, access_token: str, unique: bool = True) -> List[Vendor]:
        ...
        if isinstance(data, list):
            if unique:
                ...
                logger.info(f"供应商去重：API 返回 {len(data)} 条，去重后 {len(unique_data)} 个唯一供应商（仅管理组织）")
                return [Vendor.from_api(item) for item in unique_data]
            return [Vendor.from_api(item) for item in data]
        return []
    
    def get_vendor_detail_parsed(self, access_token: str, 
                                  vendor_id: str,
                                  org_id: Optional[str] = None) -> Optional[VendorDetail]:
        """
        查询供应商档案详情（解析为模型对象）
    ...
    @retry_on_failure()
    def query_vendors_batch(self, access_token: str,
                            vendor_ids: List[str],
                            org_id: Optional[str] = None) -> Dict:
        """
        批量查询供应商档案详情
    ...
    def query_vendors_batch_parsed(self, access_token: str,
                                   vendor_ids: List[str],
                                   org_id: Optional[str] = None) -> List[VendorDetail]:
        """
        批量查询供应商档案详情（解析为模型对象）
    ...
    @retry_on_failure()
    def query_vendors_listv3(self, access_token: str,
        ...
        body["vendorextendNeedQueryAll"] = vendor_extend_need_query_all if vendor_extend_need_query_all is not None else True
